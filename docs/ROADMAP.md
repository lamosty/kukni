...
- require successful common-image and sandboxed PDF rendering from the installed
  package; fallback resilience is a different test and must not satisfy this gate;
...
core release requirements—not optional features postponed until after release.

## 0.2 — Remaining image formats and camera RAW

Make Kukni immediately useful for the most common visual files:

- add isolated SVG and HEIC preview paths;
...
- add full-source detail where current bounded retained previews are insufficient;
...
- extend the existing information panel with privacy-aware metadata and GPS
  hidden by default;
...
## 0.5 — Public distribution and updates

- publish signed source tags and checksummed release archives;
- graduate the local alpha `.deb` into signed, published binary releases;
